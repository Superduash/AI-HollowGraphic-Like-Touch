...
class EyeTracker:
    ...
    def __init__(self, gain: float = 1.8) -> None:
        self._mesh = None
        self._eye_x, self._eye_y = -1.0, -1.0
        self._init = False
        ...
        if not _OK or mp is None:
            return
        try:
            self._mesh = mp.solutions.face_mesh.FaceMesh(  # type: ignore[attr-defined]
                max_num_faces=1, refine_landmarks=True,
                min_detection_confidence=0.5, min_tracking_confidence=0.4)
            self.available = True
            print("[EYE] Iris tracker ready")
        except Exception:
            pass
    ...
    def draw(self, frame_rgb) -> None:
        ...
        try:
            ...
            mp_drawing = mp.solutions.drawing_utils  # type: ignore[union-attr,attr-defined]
            mp_face_mesh = mp.solutions.face_mesh  # type: ignore[union-attr,attr-defined]
            face = res.multi_face_landmarks[0]
            h, w = frame_rgb.shape[:2]

            # Draw Eye Contours (Skeleton)
            spec = mp_drawing.DrawingSpec(color=(120, 255, 120), thickness=1, circle_radius=0)
            mp_drawing.draw_landmarks(
                image=frame_rgb, landmark_list=face,
                connections=mp_face_mesh.FACEMESH_LEFT_EYE, connection_drawing_spec=spec)
            mp_drawing.draw_landmarks(
                image=frame_rgb, landmark_list=face,
                connections=mp_face_mesh.FACEMESH_RIGHT_EYE, connection_drawing_spec=spec)
